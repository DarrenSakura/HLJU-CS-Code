...
def test():
    ...
    with torch.no_grad():
        for x, _ in dataloaders:
            y = model(x.to(device))
            img_y = torch.squeeze(y).detach().cpu().numpy()
            plt.imshow(img_y)
            plt.pause(0.01)
        plt.show()
...
if __name__ == '__main__':
    ...
    parser.add_argument('action', nargs='?', default='test', choices=['train', 'test'], help='train or test')#添加参数
    parser.add_argument('--batch_size', type=int, default=4)
    parser.add_argument('--weight', type=str, default='weights_19.pth', help='the path of the mode weight file')
